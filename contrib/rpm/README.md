...
The RPM spec file provided here is for KazuByte-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kazubyte.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[KazuByte git repository](https://github.com/kazubyte/kazubyte) but are not part of
...
Currently only `Source10` is used, for the example `kazubyte.conf` file.
...
in the kazubyte git repository. At present only an SVG version of the KazuByte
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kazubyte
...
needed when 0.12.1 is released, a proper fix is already in the KazuByte git
master branch.
...
idea to build KazuByte Core against a static version of that library compiled
...
The `kazubyte.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kazubyte-server` package. At this point, most RPM based
...
The `kazubyte-server` package should have SELinux support. How to properly do
...
KazuByte-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kazubyte/). He has been
packaging KazuByte for Fedora at least since 2012.
...
4. I renamed the `kazubyte` package that contains the Qt GUI to `kazubyte-core` as
that appears to be how the general population refers to it, in contrast to
`kazubyte-xt` or `kazubyte-classic`. I wanted to make sure the general population
