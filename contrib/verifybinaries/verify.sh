...
###   This script attempts to download the signature file SHA256SUMS.asc from kazubyte.org
...
WORKINGDIR="/tmp/kazubyte"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://kazubyte.org/bin/"
VERSIONPREFIX="kazubyte-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kazubyte-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the KazuByte Core binary release signing key installed?"
   fi
