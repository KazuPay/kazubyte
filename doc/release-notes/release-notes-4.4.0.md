...
- Fee to create a Proposal: 50 KBYTE
...
You can read more about [NPIP0003 on the NPIP GitHub](https://github.com/KBYTECoin/npips/blob/master/npip-0003.mediawiki).

## Community Fund Contribution Increase

The Version Bit 16 will signal for the soft fork to increase the Community Fund contribution from 0.25 KBYTE to 0.50 KBYTE per block. 

You can read more about [NPIP0005 on the NPIP GitHub](https://github.com/KBYTECoin/npips/blob/master/npip-0005.mediawiki).
...
```
createproposal kazubyteaddress amount duration "desc" fee

Arguments:
1. "kazubyteaddress" (string, required) The kazubyte address where coins would be sent if the proposal is approved.
2. "amount" The amount in KBYTE to request. eg 100
...
2. "amount" (numeric or string, required) The amount in KBYTE to withdraw. eg 10
...
Donate KBYTE from your wallet to the commnuity fund

Arguments:
1. "amount" (string, required) The amount of KBYTE to donate
```
...
### Other modifications in the KazuByte client:
