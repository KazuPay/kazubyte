...
// Copyright (c) 2017-2018 The KazuByte Core developers
...
#if defined(NDEBUG)
# error "Kazubyte cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Kazubyte Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("kazubyte-scriptch");
    ...
    for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
        ...
        if(nAccValue > nPOWBlockReward && block.vtx[0].vout[i].nValue > 0) {
            ...
            if(metadata[nPaymentRequestsCount].isStr()) {
                ...
                if(block.vtx[0].vout[i].nValue != prequest.nAmount || prequest.fState != CFund::ACCEPTED || proposal.Address != CKazuByteAddress(address).ToString())
...
//
// Commented - KazuByte uses now version control
//
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of KazuByte, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of KazuByte, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of KazuByte, please update.";
            fObsolete = true;
        }
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees, CBlockIndex* pindexPrev)
{
    ...
    if(nHeight-1 < 7 * Params().GetConsensus().nDailyBlockCount)
        ...
        nRewardCoinYear = 10.8 * MAX_MINT_PROOF_OF_STAKE; // 9% per month = 108% per year
    else if(nHeight-1 < (730 * Params().GetConsensus().nDailyBlockCount))
        nRewardCoinYear = 8.4 * MAX_MINT_PROOF_OF_STAKE; // 7% per month = 84% per year
    else if(IsCommunityFundAccumulationEnabled(pindexPrev, Params().GetConsensus(), false))
        nRewardCoinYear = 5.9 * MAX_MINT_PROOF_OF_STAKE; // 5% per month = 60% per year - 1% = 59%
    else
        nRewardCoinYear = 6.0 * MAX_MINT_PROOF_OF_STAKE; // 5% per month = 60% per year
