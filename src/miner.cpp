        ...
        if(pblocktree->GetPaymentRequestIndex(vec))
        {
            BOOST_FOREACH(const CFund::CPaymentRequest& prequest, vec) {
                ...
                if(prequest.fState == CFund::ACCEPTED && prequest.paymenthash == uint256() &&
                    ...
                    CFund::CProposal proposal;
                    if(CFund::FindProposal(prequest.proposalhash, proposal)) {
                        CKazuByteAddress addr(proposal.Address);
                        if (!addr.IsValid())
                            continue;
                        coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
...
void KazuByteStaker(const CChainParams& chainparams)
{
    LogPrintf("KazuByteStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kazubyte-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in KazuByteStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running KazuByteStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("KazuByteStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("KazuByteStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("KazuByteStaker: ProcessNewBlock, block not accepted");
        }
        else
        {
