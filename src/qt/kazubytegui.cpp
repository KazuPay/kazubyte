...
#include "main.h"
#include "kazubytegui.h"
#include "kazubyteunits.h"
...
const std::string KazuByteGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KazuByteGUI::DEFAULT_WALLET = "~Default";

KazuByteGUI::KazuByteGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void KazuByteGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/kazubyte"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a KazuByte address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kazubyte: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your KazuByte addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified KazuByte addresses"));
    ...
    openAction->setStatusTip(tr("Open a kazubyte: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible KazuByte command-line options").arg(tr(PACKAGE_NAME)));
...
void KazuByteGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://nav.nyc3.digitaloceanspaces.com/bootstrap/mainnet/bootstrap-kazubyte_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the KazuByte Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void KazuByteGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(KazuByteUnits::Unit u, KazuByteUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(KazuByteUnits::name(u)), this);
...
void KazuByteGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void KazuByteGUI::createToolBars()
{
    if(walletFrame)
    {
...
void KazuByteGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool KazuByteGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void KazuByteGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool KazuByteGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KazuByteGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KazuByteGUI::setWalletActionsEnabled(bool enabled)
{
...
void KazuByteGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KazuByteGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KazuByteGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KazuByteGUI::optionsClicked()
{
...
void KazuByteGUI::showDebugWindow()
{
...
void KazuByteGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KazuByteGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KazuByteGUI::openClicked()
{
...
void KazuByteGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void KazuByteGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuByteGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuByteGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuByteGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void KazuByteGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KazuByteGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void KazuByteGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to KazuByte network", "", count));
    if(walletFrame){
...
void KazuByteGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to KazuByte network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to KazuByte network..."));
...
void KazuByteGUI::showVotingDialog()
{
...
void KazuByteGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("KazuByte"); // default title
...
void KazuByteGUI::changeEvent(QEvent *e)
{
...
void KazuByteGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KazuByteGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KazuByteGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KazuByteUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void KazuByteGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KazuByteGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KazuByteGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KazuByteGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KazuByteGUI::setEncryptionStatus(int status)
{
...
void KazuByteGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KazuByteGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KazuByteGUI::detectShutdown()
{
...
void KazuByteGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KazuByteGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(KazuByteGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KazuByteGUI::subscribeToCoreSignals()
{
...
void KazuByteGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<KazuByteUnits::Unit> units = KazuByteUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const KazuByteUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(KazuByteUnits::name(unit)));
    }
...
{
    setText(KazuByteUnits::name(newUnits));
}
...
void KazuByteGUI::toggleStaking()
{
...
void KazuByteGUI::replyFinished(QNetworkReply *reply)
{
...
void KazuByteGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CKazuByteAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void KazuByteGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.kazubyte.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void KazuByteGUI::updateStakingStatus()
{
    updateWeight();
