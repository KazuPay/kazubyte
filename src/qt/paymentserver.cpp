...
const int KBYTECOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString KBYTECOIN_IPC_PREFIX("kazubyte:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
const char* NIP01_MESSAGE_SIGNEDMSG = "SignedMsg";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/kazubyte-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/kazubyte-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/kazubyte-paymentrequest";
const char* NIP01_MIMETYPE_SIGNEDMSG = "application/kazubyte-signedmsg";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the kazubyte: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(KBYTECOIN_IPC_PREFIX, Qt::CaseInsensitive)) // kazubyte: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseKazuByteURI(arg, &r) && !r.address.isEmpty())
            {
                CKazuByteAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(KBYTECOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(KBYTECOIN_IPC_CONNECT_TIMEOUT);
    ...
    model(0)
{
    ...
    // on Mac: sent when you click kazubyte: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            LogPrintf("Cannot start kazubyte: click-to-pay handler\n");
        }
        else {
...
//
// OSX-specific way of handling kazubyte: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in kazubyte: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("kazubyte://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'kazubyte://' is not a valid URI. Use 'kazubyte:' instead."),
                       CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(KBYTECOIN_IPC_PREFIX, Qt::CaseInsensitive)) // kazubyte: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        if (uri.hasQueryItem("m") && uri.hasQueryItem("a")) // sign message URI
        {
            ...
            CKazuByteAddress addr(uri.queryItemValue("a").toStdString());
            if (!addr.IsValid())
            {
            ...
            QUrl fetchUrlConstructed(s.mid(KBYTECOIN_IPC_PREFIX.length()));
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseKazuByteURI(s, &recipient))
            {
              ...
              if (!address.IsValid()) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid KazuByte address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CKazuByteAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom kazubyte addresses are not supported
        ...
        // KazuByte amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(KazuByteUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
