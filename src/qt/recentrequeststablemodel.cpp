    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return KazuByteUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, KazuByteUnits::separatorNever);
            else
                return KazuByteUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+KazuByteUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
