...
const char * const KBYTECOIN_CONF_FILENAME = "kazubyte.conf";
const char * const KBYTECOIN_PID_FILENAME = "kazubyte.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KazuByte
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KazuByte
    // Mac: ~/Library/Application Support/KazuByte
    // Unix: ~/.kazubyte
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KazuByte";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KazuByte";
#else
    // Unix
    return pathRet / ".kazubyte";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", KBYTECOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No kazubyte.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override kazubyte.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", KBYTECOIN_PID_FILENAME));
