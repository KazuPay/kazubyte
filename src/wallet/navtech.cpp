...
UniValue Navtech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    ...
    throw runtime_error("Unable to send KBYTETech transaction, please try again.");
  }
...
vector<anonServer> Navtech::GetAnonServers() {
  ...
  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one KBYTETech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid KBYTETech server to send private payments.");
      return returnServers;
  }
...
UniValue Navtech::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured KBYTETech nodes are available right now.");
  }
...
string Navtech::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {
  ...
  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",KBYTETECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
